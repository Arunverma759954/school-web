...
export default function NoticeBoard() {
    ...
    useEffect(() => {
        ...
        fetchNotices();
        const interval = setInterval(fetchNotices, 10000); // Auto refresh every 10s
                    ...
                    <div className="lg:col-span-7 bg-[#F5F5F5] relative overflow-hidden group">
                        <div className="flex flex-col md:flex-row gap-0">
                            ...
                            <div className="w-full md:w-2/5 relative min-h-[400px] md:min-h-[500px] flex items-center justify-center p-4">
                                {/* Decorative Element */}
                                <div className="absolute w-[80%] h-[80%] border-[3px] border-primary/20 rounded-3xl top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"></div>
                            ...
                            <div className="w-full md:w-3/5 p-8 md:p-10 lg:p-12 flex flex-col justify-center">
                                <h2 className="text-2xl md:text-3xl lg:text-4xl font-serif font-bold text-brand-red mb-2 uppercase tracking-wide">
                                    Welcome To
                                </h2>
                                <h3 className="text-xl md:text-2xl lg:text-3xl font-serif font-bold text-brand-red mb-2 uppercase tracking-wide">
                                    St. Joseph's Convent
                                </h3>
                                <h3 className="text-xl md:text-2xl lg:text-3xl font-serif font-bold text-brand-red mb-2 uppercase tracking-wide">
                                    School
                                </h3>
                                <h4 className="text-lg md:text-xl lg:text-2xl font-serif font-bold text-brand-red mb-6 uppercase tracking-wide">
                                    Jharsuguda
                                </h4>
                                ...
                                <div className="flex justify-end">
                                    <button className="bg-brand-red text-white px-8 py-3 font-bold uppercase tracking-widest text-sm hover:bg-black transition-all shadow-lg rounded-sm">
                                        Learn More
                                    </button>
                                </div>
                            </div>
                                ...
                                notices.map((notice, idx) => (
                                    <div key={idx} className="group border-b border-gray-100 last:border-0 pb-4 hover:translate-x-2 transition-all duration-300">
                                        <div className="flex items-start gap-4">
                                            <div className="bg-primary/5 p-3 rounded-2xl text-primary font-black text-center min-w-[70px]">
                                            ...
                                            <div>
                                                ...
                                                <h4 className="font-bold text-gray-900 mt-1 line-clamp-2">
                                                    {notice.link ? (
                                                        <a href={notice.link} target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors flex items-center gap-1">
